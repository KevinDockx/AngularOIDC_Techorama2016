...
namespace TripGallery.API.UnitOfWork.Trip
{
    public class GetTrips : IUnitOfWork<IEnumerable<DTO.Trip>>, IDisposable
    {
        ...
        {
            IQueryable<Repository.Entities.Trip> tripEntities = 
                new List<Repository.Entities.Trip>().AsQueryable();

            tripEntities = _ownerId == null
                ? _tripRepository.GetTrips() :_tripRepository.GetTrips(_ownerId);
